...
##Testing to see if there is a difference between contexts

```{r} 
wilcox.test(Fair ~ Context , data = dc2)$p.value
wilcox.test(Acc ~ Context , data = dc2)$p.value
wilcox.test(Sat ~ Context , data = dc2)$p.value
wilcox.test(Useful ~ Context , data = dc2)$p.value
wilcox.test(Clear ~ Context, data = dc2)$p.value
wilcox.test(Meaningful ~Context , data = dc2)$p.value

```

Yes, there is a statistically significant difference between answers across contexts for all variables. 

##Testing to see if there is a difference in the differences between contexts 
```{r} 
# dc3<-dc2
# dc3$dFair<-dc3$Fair[dc3$treat==1]-dc3$Fair[dc3$treat==0]
# dc3$dAcc<-dc3$Acc[dc3$treat==1]-dc3$Acc[dc3$treat==0]
# dc3$dSat<-dc3$Sat[dc3$treat==1]-dc3$Sat[dc3$treat==0]
# dc3$dUseful<-dc3$Useful[dc3$treat==1]-dc3$Useful[dc3$treat==0]
# dc3$dClear<-dc3$Clear[dc3$treat==1]-dc3$Clear[dc3$treat==0]
# dc3$dMeaningful<-dc3$Meaningful[dc3$treat==1]-dc3$Meaningful[dc3$treat==0]
# 
# wilcox.test(dFair ~ Context , data = dc3)$p.value
# wilcox.test(dAcc ~ Context , data = dc3)$p.value
# wilcox.test(dSat ~ Context , data = dc3)$p.value
# wilcox.test(dUseful ~ Context , data = dc3)$p.value
# wilcox.test(dClear ~ Context, data = dc3)$p.value
#wilcox.test(dMeaningful ~Context , data = dc3)$p.value

#I'm not sure if this is correct to look at the difference between the change between treatment and control in twitter vs. recidivism. It doesn't seem quite right.
